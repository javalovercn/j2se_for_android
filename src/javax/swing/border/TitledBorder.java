...
public class TitledBorder extends AbstractBorder
{
    ...
    public TitledBorder(Border border,
    	...
    	this.titleFont = titleFont==null?UICore.getDefaultDialogInputFontForSystemUIOnly():titleFont;
    ...
    private void buildInitView(){
    	...
    	titleView = new TextView(ActivityManager.getActivity());
    ...
    public void setTitle(String title) {
    	...
    	insets = new Insets(isNoTitle?pixel:AndroidUIUtil.getDefaultTextViewHeight(this.titleFont, screenAdapter), 
        		pixel, pixel, pixel);
    }
...
//					lp.setMargins(pixel, titleHeight/2, pixel, pixel);//+3增加底部，防止与底框过近
		        	lp.setMargins(insets.left, titleHeight, insets.right, insets.bottom);
					lp.gravity = (Gravity.TOP | (component.getComponentOrientation().isLeftToRight()?Gravity.LEFT:Gravity.RIGHT));//(Gravity.CENTER | Gravity.CENTER_HORIZONTAL | Gravity.CENTER_HORIZONTAL);
		//			System.out.println("set Frame Margins l:" + lp.leftMargin + ", t:" + lp.topMargin + ", r:" + lp.rightMargin + ", b:" + lp.bottomMargin);
				...
				{
			    	LinearLayout boxView = new LinearLayout(ActivityManager.getActivity());
		        	boxView.setBackgroundResource(HCRUtil.getResource(HCRUtil.R_drawable_border_titled));
	...
	private void addTitleView(Component component){
		if(title == null || title.trim().length() == 0){
    		...
    		lp.setMargins(
    				insets.left + (isLeftToRight?AndroidUIUtil.INDENT_PIXEL:0), 0, 
    				insets.right + (isLeftToRight==false?AndroidUIUtil.INDENT_PIXEL:0), insets.bottom);
