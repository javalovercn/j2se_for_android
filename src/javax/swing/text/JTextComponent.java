...
public abstract class JTextComponent extends JComponent implements Scrollable, Accessible
{
    ...
    public JTextComponent() {
        ...
        AndroidUIUtil.runOnUiThreadAndWait(new Runnable() {
			@Override
			public void run() {
		        editText = new EditText(ActivityManager.getActivity());
			}
		});
...
//        editText.setText(((DefaultStyledDocument)model).getSpanableContentAdAPI().spanBuilder, TextView.BufferType.EDITABLE);
        ...
        editText.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				try {
					...
					if(count > 0){
						model.insertString(start, s.subSequence(start, start + count).toString(), null);
					}
//					System.out.println("Result : " + JTextComponent.this.getText());
				} catch (Exception e) {
					e.printStackTrace();
				}
...
//							"[onTextChanged start : "+start+", before : "+before+", count : "+count+"]");
					...
					for (int i = 0; i < listeners.length; i++) {
						KeyEvent event = new KeyEvent(JTextComponent.this, 
								KeyEvent.KEY_RELEASED, curMS, 0, keyCode);
			...
			public void beforeTextChanged(CharSequence s, final int start, int count,
				...
				if(listeners != null && listeners.length > 0){
//					DebugLogger.warning("create KeyEvent for " +
//							"[beforeTextChanged start : "+start+", after : "+after+", count : "+count+"]");
					long curMS = System.currentTimeMillis();
					char keyCode = s.charAt(start);
					for (int i = 0; i < listeners.length; i++) {
						KeyEvent event = new KeyEvent(JTextComponent.this, 
								KeyEvent.KEY_PRESSED, curMS, 0, keyCode);
						listeners[i].keyTyped(event);
					}
				}
			...
			public void afterTextChanged(Editable s) {
				...
				KeyListener[] listeners = JTextComponent.this.getKeyListeners();
				if(listeners != null && listeners.length > 0){
//					DebugLogger.warning("create KeyEvent for " + "[afterTextChanged]");
					for (int i = 0; i < listeners.length; i++) {
						KeyEvent event = new KeyEvent(JTextComponent.this, 
            ...
            public boolean onEditorAction(TextView v, int actionId, android.view.KeyEvent event) {  
                if (actionId == EditorInfo.IME_ACTION_DONE) {  
                	...
                	if(id != android.view.View.NO_ID) {
                        ActivityManager.getActivity().findViewById(id).requestFocus();
                    	return true;
                    }
//                    InputMethodManager imm = (InputMethodManager)v.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);  
//                    imm.hideSoftInputFromWindow(v.getWindowToken(), 0);  
//                	sendKeyCode(android.view.KeyEvent.KEYCODE_DPAD_DOWN);
		...
		
		{
			...
			editText.setTextColor(isEnable?AndroidUIUtil.WIN_FONT_COLOR.toAndroid():AndroidUIUtil.WIN_FONT_DISABLE_COLOR.toAndroid());
...
//		editText.setText(((DefaultStyledDocument)model).getSpanableContentAdAPI().spanBuilder, TextView.BufferType.SPANNABLE);
//	}
    ...
    public void copy() {
    	if(editText.isSelected()){
	    	android.content.ClipboardManager clipboardManager = (android.content.ClipboardManager)ActivityManager.getActivity().getSystemService(Context.CLIPBOARD_SERVICE);  
...
//    	}
    ...
    public void setEnabled(boolean enable){
    	...
    	editText.setTextColor(enable?AndroidUIUtil.WIN_FONT_COLOR.toAndroid():AndroidUIUtil.WIN_FONT_DISABLE_COLOR.toAndroid());
    ...
    private void setShowSoftInputOnFocus(final EditText ed, final boolean isShow){
    	ActivityManager.getActivity().runOnUiThread(new Runnable() {
			@Override
			public void run() {
		    	int mode = isShow?WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN:WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE;
		        ActivityManager.getActivity().getWindow().setSoftInputMode(mode); 
    ...
    public void setEditable(boolean b) {
		...
		editText.setTextColor(editable?AndroidUIUtil.WIN_FONT_COLOR.toAndroid():AndroidUIUtil.WIN_FONT_DISABLE_COLOR.toAndroid());
    	setKeyNull(b);
    ...
    public final void setTextBySpanBuilder(final String snapText) {
		final SpannableStringBuilder spanBuilder = ((DefaultStyledDocument)model).getSpanableContentAdAPI().spanBuilder;
		spanBuilder.clear();
