...
public abstract class ImageReader {
	 ...
	 * this.minIndex</code>; otherwise, the value of <code>minIndex</code> will
	 * be set to <code>index</code>. If <code>seekForwardOnly</code> is
	 * <code>false</code>, the value of <code>minIndex</code> will remain 0
	 * regardless of any read operations.
	 *
	 * <p>
	 ...
	 * {@link #isImageTiled <code>isImageTiled</code>}.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The resuting metadata object is only responsible for returning documents
	 ...
	 * for each image decoded, there will be a call to
	 * <code>imageStarted</code>, followed by calls to
	 * <code>imageProgress</code> as the read progresses, and finally to
	 * <code>imageComplete</code>. The <code>sequenceComplete</code> method will
	 * be called after the last image has been decoded.
	 * <code>IIOReadUpdateListener</code> objects may be updated at other times
	 * during the read as pixels are decoded. Finally,
	 * <code>IIOReadWarningListener</code> objects will receive notification of
	 * any non-fatal warnings that occur during decoding.
	 ...
	 * {@link java.awt.image.Raster <code>Raster</code>} of pixel data. If this
	 * method returns <code>false</code>, calls to {@link #readRaster
	 * <code>readRaster</code>} or {@link #readTileRaster
	 ...
	 * are used exactly as in the {@link #read <code>read</code>} method, except
	 * that any destination offset is used as a logical rather than a physical
	 * offset. The size of the returned <code>Raster</code> will always be that
	 * of the source region clipped to the actual image. Logical offsets in the
	 * stream itself are ignored.
	 *
	 * <p>
	 ...
	 * as they are for the {@link #read <code>read</code>} method.
	 *
	 * <p>
	 ...
	 * <code>read(int, ImageReadParam)</code> with a read param specifiying a
	 ...
	 * supported, and if so calls {@link #readRaster
	 * <code>readRaster(imageIndex, null)</code>} if <code>tileX</code> and
	 * <code>tileY</code> are 0, or throws an
	 * <code>IllegalArgumentException</code> otherwise.
	 *
	 * @param imageIndex
	 *            the index of the image to be retrieved.
	...
	// *
	// * @param listener an <code>IIOReadWarningListener</code> to be
	// registered.
	// *
	 ...
	 * <code>computeRegions</code>} method performs all necessary clipping.
	 *
	 * @param param
