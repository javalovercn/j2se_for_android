...
//	final int titleBaseHeight = 14;
	...
	public HCDesktop(final Image image, String toolTip, final JPopupMenu menu){
		...
		statusIconView = new ImageView(ActivityManager.getActivity());
		messageView = new TextView(ActivityManager.getActivity());
	...
	public void showDesktop(){
		LinearLayout desktopLinear = new LinearLayout(ActivityManager.getActivity());
		desktopLinear.setOrientation(LinearLayout.VERTICAL);
		{
			messageLinear = new LinearLayout(ActivityManager.getActivity());
			messageLinear.setBackgroundColor(AndroidUIUtil.WINDOW_DESKTOP_UP_DOWN_COLOR.toAndroid());
	...
	public void displayMessage(final String caption, final String message, final MessageType messageType) {
		ActivityManager.getActivity().runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				LinearLayout titleBar = new LinearLayout(ActivityManager.getActivity());
				{
					{
					...
					{
						TextView captionTV = new TextView(ActivityManager.getActivity());
						captionTV.setText(caption);
						captionTV.getPaint().setFakeBoldText(true);
						UICore.setTextSize(captionTV, UICore.buildDefaultDialogFont(), J2SEInitor.getAndroidServerScreenAdapter());
...
//				titleBar.setBackgroundColor(UIUtil.WIN_BODY_BACK.toAndroid());
				...
				{
					TextView messageTV = new TextView(ActivityManager.getActivity());
...
//					messageTV.setBackgroundColor(UIUtil.WIN_BODY_BACK.toAndroid());
				...
				Toast toast = new Toast(ActivityManager.getActivity());
	...
	public void setImage(final Image image) {
		...
		ActivityManager.getActivity().runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				int scaleSize = J2SEInitor.getAndroidServerScreenAdapter().imageSizeToScreenFloat(iconBaseHeight, 1.0F);
				statusIconView.setImageBitmap(ImageUtil.zoomBitmap(image.getBitmapAdAPI(), scaleSize, scaleSize));
