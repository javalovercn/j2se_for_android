...
public class AndroidMultiThreadDownloader {
	private ProgressBar progress = new ProgressBar(ActivityManager.applicationContext, null,
	...
	public void download(final Vector url_download, final File file, final String md5,
		...
		final String desc_str = buildDownloadMsg(dnFileName, fileName, md5, downloadByte,
				totalByted, startMS);
		...
		refreshProgress = new Thread() {
			...
			public void run() {
				...
				while (isError == false && isCancel == false && (downloadByte < totalByted)) {
					...
					final String desc_str = buildDownloadMsg(dnFileName, fileName, md5,
	...
	private String buildDownloadMsg(String fromURL, String storeFile, String md5, int readed,
			int total, long startMS) {
		...
		out += "<STRONG>Downloaded :    " + String.format("%.2f", readedM) + "M from "
				+ String.format("%.2f", totalM) + "M, " + ((int) process) + "%</STRONG><BR><BR>";
...
class DownloadThread extends Thread {
	...
	public DownloadThread(int threadId, AndroidMultiThreadDownloader main, int start, int end,
			File file, URL url) {
		this.main = main;
